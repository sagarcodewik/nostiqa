...
const SHEET_RANGE = "FoundingPartner!A:L";
...
module.exports = async function handler(req, res) {
    ...
    const creatorUsage = typeof body.creatorUsage === "string" ? body.creatorUsage.trim() : "";
    ...
    if (!EMAIL_PATTERN.test(email)) {
        res.status(422).json({ success: false, message: "Please enter a valid work email address." });
        return;
    }

    if (!isValidUrl(website)) {
        res.status(422).json({ success: false, message: "Please enter a valid website URL." });
        return;
    }

    const shortFields = { name, email, company, website, industry, creatorUsage, productInterest };
    ...
    if (objective.length > MAX_LONG_FIELD_LENGTH || notes.length > MAX_LONG_FIELD_LENGTH) {
        res.status(422).json({
            success: false,
            message: `Objective and notes must be under ${MAX_LONG_FIELD_LENGTH} characters.`,
        });
        return;
    }
    ...
    if (!spreadsheetId || !serviceAccountJson) {
        console.error("[api/submit-founding-partner] Missing GOOGLE_SHEET_ID or GOOGLE_SERVICE_ACCOUNT_JSON env var.");
    ...
    try {
        ...
        console.error("[api/submit-founding-partner]", error);
        res.status(500).json({ success: false, message: "Unable to save your submission. Please try again later." });
    }
};
